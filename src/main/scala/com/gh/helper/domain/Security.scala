 ...
 */
case class Security(userId: String, fbToken: String, userToken: String, createDate: Option[Timestamp])


class Securities(tag: Tag) extends Table[Security](tag, "security") {

  def userId = column[String]("userId", O.PrimaryKey, O.Length(64))
  ...
  def * = (userId, fbToken, userToken, createDate.?) <> ((Security.apply _).tupled, Security.unapply)
